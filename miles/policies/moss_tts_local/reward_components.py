...
def parse_components(raw: str | None) -> tuple[RewardComponent, ...]:
    ...
    for entry in value.replace(",", " ").split():
        ...
        if canonical not in _KNOWN_COMPONENTS:
            raise ValueError(
                f"Unknown MOSS-TTS reward component {name!r}; expected wer, sim, or rm."
            )
        try:
            weight = float(raw_weight)
        except ValueError as error:
            raise ValueError(
                f"Reward component {entry!r} has a non-numeric weight."
            ) from error
        if not math.isfinite(weight) or weight < 0:
            raise ValueError(
                f"Reward component {entry!r} must have a finite non-negative weight."
            )
    ...
    if total <= 0 or not math.isclose(total, 1.0, rel_tol=0.0, abs_tol=1e-6):
        raise ValueError(
            f"MOSS-TTS reward component weights must sum to 1, got {total}."
        )
    return tuple(parsed)
